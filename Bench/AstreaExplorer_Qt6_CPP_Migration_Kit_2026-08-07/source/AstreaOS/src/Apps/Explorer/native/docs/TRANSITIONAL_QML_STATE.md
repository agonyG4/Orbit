...
This inventory is intentionally descriptive. Phase 4 keeps the existing QML
...
| Candidate | Current responsibility | Native replacement / owner | Phase 4 status |
| --- | --- | --- | --- |
...
| `state/RecentState.qml` (`Process` at lines 145, 182) | fallback recent item load/save through the helper | `RecentController` and `RecentStore` | `NATIVE_AUTHORITATIVE`; QML helper retained only for fallback and guarded when native navigation is active |
...
these imports in Phase 4; the remaining QML modules stay available for fallback
and for domains not yet migrated.
...
3. Move preview/launch and portal flows behind their existing native owners,
