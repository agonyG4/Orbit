...
class RealMainQmlTest final : public QObject
{
    Q_OBJECT

private slots:
    void nativeSelfTestLoadsExistingMainQml();
};

void RealMainQmlTest::nativeSelfTestLoadsExistingMainQml()
{
    ...
    if (output.contains("missing required capabilities")) {
        QSKIP(qPrintable(QStringLiteral("Explorer runtime capabilities are unavailable: %1")
    ...
    QVERIFY2(
        ...
        qPrintable(QStringLiteral("Native self-test did not load existing Main.qml: %1")
                       .arg(QString::fromUtf8(output))));
}

QTEST_MAIN(RealMainQmlTest)

#include "tst_real_main_qml.moc"
