...
class ExplorerArchiveAdmissionTests(unittest.TestCase):
    ...
    def test_archive_menu_uses_capabilities_profiles_and_real_extract_actions(self):
        ...
        self.assertIn("function runExtractHere()", menu)
        self.assertIn("AppState.startArchiveExtractionHere(itemPath, AppState.currentPath)", menu)
        self.assertIn("function runExtractToNamedFolder()", menu)
        self.assertIn("AppState.startArchiveExtraction(itemPath, extractionFolderName())", menu)
        self.assertIn('mode: "select_folder"', menu)
        self.assertIn("AppState.startArchiveExtractionTo", menu)
        self.assertIn("function canonicalArchiveStem(name)", (APP_ROOT / "AppState.qml").read_text(encoding="utf-8"))
        self.assertIn("model: {", menu)
        self.assertIn("return capability ? capability.profiles : []", menu)
        self.assertIn("validArchiveName", menu)
...
class ExplorerIconRenderingRegressionTests(unittest.TestCase):
    ...
    def test_preview_surfaces_report_native_ranges_and_physical_decode_sizes(self):
        ...
        self.assertIn("onEffectiveDprChanged: selectedPreviewTimer.restart()", panel)
        self.assertIn("onSelectedPathChanged: selectedPreviewTimer.restart()", panel)
        self.assertNotIn("selectedPreviewUrl !== \"\"", panel)
        ...
        self.assertIn("onEffectiveDprChanged: previewScheduleTimer.restart()", icon_view)
        self.assertIn("onEffectiveDprChanged: previewScheduleTimer.restart()", list_view)
